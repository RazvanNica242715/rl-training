...
ROBOTICS_DIR = Path(__file__).parent.resolve()
...
class Simulation:
    def __init__(self, num_agents, render=True, rgb_array=False):
        ...
        if render:
            ...
            mode = p.DIRECT  # for non-graphical version        # Set up the simulation
        ...
        # Load textures
        # Load textures
        texture_dir = ROBOTICS_DIR / "textures"
        ...
        # Now these will match: 01.png <-> first plate alphabetically, etc.
        random_texture = texture_list[random_index]
        random_plate = plate_list[random_index]

        self.plate_image_path = str(plates_dir / random_plate)
        self.textureId = p.loadTexture(str(texture_dir / random_texture))

        # print(f'textureId: {self.textureId}')

        # Set the camera parameters
        cameraDistance = 1.1 * (
            math.ceil((num_agents) ** 0.3)
        ...
        cameraTargetPosition = [
            -0.2,
            -(math.ceil(num_agents**0.5) / 2) + 0.5,
            0.1,
        ]  # XYZ coordinates of the target position
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if agent_count < num_agents:  # Check if more agents need to be placed
                    ...
                    robotId = p.loadURDF(
                        str(ROBOTICS_DIR / "ot_2_simulation_v6.urdf"),
                        position,
                        [0, 0, 0, 1],
                        flags=p.URDF_USE_INERTIA_FROM_FILE,
                    )
                    start_position, start_orientation = p.getBasePositionAndOrientation(
                        robotId
                    )
                    p.createConstraint(
                    ...
                    planeId = p.loadURDF(
                        str(ROBOTICS_DIR / "custom.urdf"),
                        position_with_offset,
                        rotate_90,
                    )  # start_orientation)
                    # Disable collision between the robot and the specimen
                    p.setCollisionFilterPair(
                        robotId, planeId, -1, -1, enableCollision=0
                    )
                    spec_position, spec_orientation = p.getBasePositionAndOrientation(
                        planeId
                    )
    ...
    def run(self, actions, num_steps=1):
        ...
        for i in range(num_steps):
            ...
            if self.rgb_array:
                ...
                width, height, rgbImg, depthImg, segImg = p.getCameraImage(
                    ...
                    viewMatrix=p.computeViewMatrix(
                        camera_pos, camera_target, up_vector
                    ),
            ...
            if self.render:
                time.sleep(1.0 / 240.0)  # slow down the simulation
    ...
    def drop(self, robotId):
        ...
        visualShapeId = p.createVisualShape(
            shapeType=p.GEOM_SPHERE, radius=sphereRadius, rgbaColor=sphereColor
        )
    ...
    def check_contact(self, robotId, specimenId):
        for sphereId in self.sphereIds:
            ...
            if contact_points_specimen:
                ...
                p.setCollisionFilterPair(
                    sphereId, specimenId, -1, -1, enableCollision=0
                )
                # logging.info(f'sphereId: {sphereId}, collision disabled')
                # Get current position and orientation of the sphere
                sphere_position, sphere_orientation = p.getBasePositionAndOrientation(
                    sphereId
                )
                ...
                if f"specimenId_{specimenId}" in self.droplet_positions:
                    self.droplet_positions[f"specimenId_{specimenId}"].append(
                        sphere_position
                    )
                else:
                    self.droplet_positions[f"specimenId_{specimenId}"] = [
                        sphere_position
                    ]
