...
class PIDController:
    ...
    def reset(self):
        """Reset the contrroller state."""
        self.integral = 0.0
        self.prev_error = 0.0

    def compute(
        self, error: float, dt: float, output_limits: tuple | None = None
    ) -> float:
        """
        ...
        if output_limits is not None:
            ...
            elif (output > output_limits[1] and error < 0) or (
                output < output_limits[0] and error > 0
            ):
